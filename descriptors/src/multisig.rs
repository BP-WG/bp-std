...
use amplify::num::u4;
use derive::{
    Derive, DeriveCompr, DerivedScript, KeyOrigin, Keychain, LegacyPk, NormalIndex, OpCode,
    SigScript, TapDerivation, Terminal, Witness, WitnessScript, XOnlyPk, XkeyOrigin, XpubAccount,
    XpubDerivable,
};
...
impl<K: DeriveCompr> WshSortedMulti<K> {
    ...
    fn check_sigs(&self, keysigs: &HashMap<&KeyOrigin, LegacyKeySig>) -> bool {
        let set = self
            .keys()
            .map(K::xpub_spec)
            .map(XpubAccount::origin)
            .map(XkeyOrigin::master_fp)
            .collect::<BTreeSet<_>>();
...
impl<K: DeriveCompr> Derive<DerivedScript> for WshSortedMulti<K> {
    ...
    fn derive(
        ...
        for xkey in &self.keys {
            let key = xkey.derive(keychain, index).next().expect("no derivation found");
...
{
    fn class(&self) -> SpkClass { panic!("SortedMulti::class() can not be called") }
